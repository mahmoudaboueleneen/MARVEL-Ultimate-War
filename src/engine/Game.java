...
import views.NameSelectionWindow;
...
public class Game {
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH)
				...
				if (board[currx][curry] instanceof Cover) {
					...
					getCurrentChampion().setCurrentActionPoints(getCurrentChampion().getCurrentActionPoints() - 2); //addition to the code, wasn't present in the original solution
					if (curhp <= 0)
						board[currx][curry] = null;
					return;
	...
	public static void main(String[] args) {
		//The loadChampions and loadAbilities methods should be called at the start of runtime
		
		//but first, we must create the arrays to not get null errors
		availableChampions = new ArrayList<Champion>();
		availableAbilities = new ArrayList<Ability>();
		...
		//Load Abilities
				try {
					...
					JOptionPane.showMessageDialog(null, "Error(s) encountered while loading abilities from csv file", "Error", JOptionPane.ERROR_MESSAGE); //parent component, message, title, optionType
				}
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Error(s) encountered while loading champions from csv file", "Error", JOptionPane.ERROR_MESSAGE); //parent component, message, title, optionType 
		}
